...
// Note: Students and classes are now loaded from database in component
...
const AttendanceMarking = () => {
  ...
  const loadStudentsAndClasses = async () => {
    try {
      ...
      // Build class list from streams table
      ...
      if (!streamsError && streams) {
        const classList = streams.map((stream: any) => {
          const studentsInStream = formattedStudents.filter(s => s.stream === stream.id);
          return {
            id: stream.id,
            name: (stream.name || stream.id).replace('-', ' - '),
            class_id: stream.class_id || stream.id.split('-')[0],
            totalStudents: studentsInStream.length
          };
        });
        setRealClasses(classList);
      }
    } catch (error) {
      ...
      >
        <div className="flex items-center justify-center min-h-96">
          <div className="text-center space-y-4">
            ...
            <p className="text-muted-foreground">Loading students and classes...</p>
          </div>
        </div>
      </DashboardLayout>
    );
